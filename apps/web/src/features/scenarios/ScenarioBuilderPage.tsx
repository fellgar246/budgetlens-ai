...
export function ScenarioBuilderPage({ scenarioId }: { scenarioId?: string }) {
  ...
  const payload = useMemo(
    () =>
      rules.map((rule, index) => ({
        ...
        value: rule.operation === "percentage_change" ? percentToRatio(rule.value) : `${rule.value}.0000`,
        scope: {
  ...
  useEffect(() => {
    ...
    void getScenario(apiBaseUrl(), sessionAuth(userId, organizationId), scenarioId).then((result) => {
      setName(result.data.name);
      setBaseline(result.data.budget_version_id ?? "");
    });
  }, [organizationId, scenarioId, userId]);
    ...
    const timer = window.setTimeout(() => {
      void previewScenario(apiBaseUrl(), sessionAuth(userId, organizationId), {
        ...
        .then((result) => {
          ...
          trackEvent("scenario_previewed", { operation: rules[0]?.operation ?? "percentage_change" });
        })
    ...
    <CapabilityGate allowed={capabilities.can_create_scenario} hasSession={hasSession}>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[400px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-surface border border-border bg-surface p-6">
          ...
          <Button variant="secondary" onClick={() => setRules((current) => [...current, { ...EMPTY_RULE }])}>
            {copy.addRule}
          </Button>
          <Button
